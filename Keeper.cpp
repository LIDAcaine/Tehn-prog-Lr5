...
void Keeper::Add() {
	try {
		...
		switch (Choose()) {
		case 1:
			temp[size] = new Speaker; 
			break;
		case 2:
			temp[size] = new Admin;
			break;
		case 3:
			temp[size] = new Program;
			break;
		}
...
void Keeper::Add(int _type, ifstream& fin) {
	...
	switch (_type) {
	case 1:
		temp[size] = new Speaker(fin);//speaker
		break;
	case 2:
		temp[size] = new Admin(fin);
		break;
	case 3:
		temp[size] = new Program(fin);
		break;
	}
	::count++; 
	size++;
...
void Keeper::Del() {
	...
	for (int i = 0; i < size; i++) {
		switch (Value[i]->GetType()) {
		case 1:
			cout << "[" << i + 1 << "] Выступающий." << endl;
			break;
		case 2:
			cout << "[" << i + 1 << "] Администратор." << endl;
			break;
		case 3:
			cout << "[" << i + 1 << "] Программа." << endl;
			break;
		default:
			break;
		}
